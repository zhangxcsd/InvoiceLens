...
2. **其余基于 18 位统一码前缀的规则（`SC-SELF` 除外），一律要求「名称长度 ≥4」**：在必须用编号匹配时，增加最低限度的名称形态门槛，降低证件号误配；若个别主体名称少于 4 字且确有统一码，可走人工复核或后续单独规则。
3. **`SC-SELF`（前缀 `92`）**：要求**名称长度 ≤4**（超过 4 字不按个体户机读命中）。与第 2 条组合后，个体户机读仅覆盖「短字号 + 92」场景；更短或恰好 4 字与「18 位全数字末位为数字」等仍存在弱分区，见配置内 `matching_remarks`。
...
| SC-SELF | **名称 ≤4 字**，且 18 位且前缀 `92`（超过 4 字不按个体户命中） |
